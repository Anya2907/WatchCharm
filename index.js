(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const o of e.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function n(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function s(t){if(t.ep)return;t.ep=!0;const e=n(t);fetch(t.href,e)}})();const r="/WatchCharm/assets/icons-zpEVHya5.svg#icon-facebook",i="/WatchCharm/assets/icons-zpEVHya5.svg#icon-instagram",h="/WatchCharm/assets/icons-zpEVHya5.svg#icon-youtube",y="/WatchCharm/assets/icons-zpEVHya5.svg#icon-menu-burger",T="/WatchCharm/assets/icons-zpEVHya5.svg#icon-close",D=document.querySelector(".headerIconFacebook");function I(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${r}"></use>
            </svg>
          </a>`}D.innerHTML=I();const B=document.querySelector(".headerIconInstagram");function S(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${i}"></use>
            </svg>
          </a>`}B.innerHTML=S();const E=document.querySelector(".headerIconYoutube");function L(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${h}"></use>
            </svg>
          </a>`}E.innerHTML=L();const V=document.querySelector(".headerBurger");function R(){return`<svg class="menu-burger-icon" width="34" height="34">
          <use href="${y}"></use>
        </svg>`}V.innerHTML=R();const G=document.querySelector(".footerIconFacebook");function H(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${r}"
                    ></use>
                  </svg>
                </a>`}G.innerHTML=H();const Z=document.querySelector(".footerIconInstagram");function Q(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${i}"
                    ></use>
                  </svg>
                </a>`}Z.innerHTML=Q();const z=document.querySelector(".footerIconYoutube");function P(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${h}"
                    ></use>
                  </svg>
                </a>`}z.innerHTML=P();const q=document.querySelector(".mobileMenuCloseIcon");function K(){return`<svg class="mobile-menu-close-icon" width="34" height="34">
        <use href="${T}"></use>
      </svg>`}q.innerHTML=K();const U=document.querySelector(".mobileMenuIconFacebook");function J(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${r}"></use>
          </svg>
        </a>`}U.innerHTML=J();const Y=document.querySelector(".mobileMenuIconInstagram");function j(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${i}"></use>
          </svg>
        </a>`}Y.innerHTML=j();const X=document.querySelector(".mobileMenuIconYoutube");function F(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${h}"></use>
          </svg>
        </a>`}X.innerHTML=F();const N="/WatchCharm/assets/photo-desktop-DZgTKgeg.png",O="/WatchCharm/assets/photo-desktop@2x-DFzG8mW0.png",$="/WatchCharm/assets/photo-tablet-BmO2Vl-6.png",_="/WatchCharm/assets/photo-tablet@2x-EcloSsxX.png",m="data:image/png;base64,",tt="/WatchCharm/assets/photo-mobile@2x-CYpZwyro.png",et=document.querySelector(".hero-photo");function at(){return`<source
            srcset="
              ${N} 1x,
              ${O} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${$} 1x,
              ${_} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${m} 1x,
              ${tt} 2x
            "
            media="(min-width: 375px)"
          />
          <img src="${m}" alt="" />`}et.innerHTML=at();const st="/WatchCharm/assets/watch-desktop-BoveKM56.jpg",ct="/WatchCharm/assets/watch-desktop@2x-CN_DlGJv.jpg",nt="/WatchCharm/assets/watch-tablet-DzWGp1S_.jpg",ot="/WatchCharm/assets/watch-tablet@2x-h8tUE2WX.jpg",l="/WatchCharm/assets/watch-mobile-gUDyuoK7.jpg",rt="/WatchCharm/assets/watch-mobile@2x-DC3-hsv0.jpg",it=document.querySelector(".about-us-watch");function ht(){return` <source
        srcset="
          ${st} 1x,
          ${ct} 2x
        "
        media="(min-width: 1280px)"
      />
      <source
        srcset="
          ${nt} 1x,
          ${ot} 2x
        "
        media="(min-width: 768px)"
      />
      <source
        srcset="
        ${l} 1x,
        ${rt} 2x"
        media="(min-width: 375px)"
      />
      <img src="${l}" alt="" />`}it.innerHTML=ht();const mt="/WatchCharm/assets/garmin-fenix-7X-BGyNroPE.png",lt="/WatchCharm/assets/garmin-fenix-7X@2x-CLNKzvYI.png",At="/WatchCharm/assets/garmin-fenix-7X-BYzg8P9N.png",ut="/WatchCharm/assets/garmin-fenix-7X@2x-DgvqIYlC.png",A="/WatchCharm/assets/garmin-fenix-BtUZFNqa.png",gt="/WatchCharm/assets/garmin-fenix@2x-DgAmOKLa.png",pt="/WatchCharm/assets/garmin-marq-D7Zoid31.png",xt="/WatchCharm/assets/garmin-marq@2x-D2WqiF2K.png",dt="/WatchCharm/assets/garmin-marq-_WDLzsmf.png",bt="/WatchCharm/assets/garmin-marq@2x-BazMqD_o.png",u="/WatchCharm/assets/garmin-marq-S2plHlxp.png",Mt="/WatchCharm/assets/garmin-marq@2x-CgjFqqcK.png",wt="/WatchCharm/assets/garmin-descent-CatdJNn4.png",v="/WatchCharm/assets/garmin-D2@2x-CTMGRPFG.png",kt="/WatchCharm/assets/garmin-descent-RgCLEepd.png",Wt="/WatchCharm/assets/garmin-descent@2x-DJlmk73G.png",g="/WatchCharm/assets/garmin-descent-D058-41F.png",ft="/WatchCharm/assets/garmin-descent@2x-DSrl5rur.png",Ct="/WatchCharm/assets/garmin-D2-BPYVYLKf.png",vt="/WatchCharm/assets/garmin-D2-CdkTsl9X.png",yt="/WatchCharm/assets/garmin-D2@2x-uwK-wnBs.png",p="/WatchCharm/assets/garmin-D2-B_lN0l0e.png",Tt="/WatchCharm/assets/garmin-D2@2x-D4h7HLtv.png",Dt="/WatchCharm/assets/garmin-fenix-6-_fQUo3lG.png",It="/WatchCharm/assets/garmin-fenix-6@2x-Du5HDKBW.png",x="/WatchCharm/assets/garmin-fenix-6-D5xVD_H4.png",Bt="/WatchCharm/assets/garmin-fenix-6@2x-DmzbNT7X.png",St="/WatchCharm/assets/tube-watch-S42-ba4c6ajU.png",Et="/WatchCharm/assets/tube-watch-S42@2x-CaBP822J.png",d="/WatchCharm/assets/tube-watch-S42-CK21dafh.png",Lt="/WatchCharm/assets/tube-watch-S42@2x-DFR-76cY.png",Vt=document.querySelector(".catalogGarminFenix7X");function Rt(){return`<source
            srcset="
              ${mt} 1x,
              ${lt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${At} 1x,
              ${ut} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${A} 1x,
              ${gt} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${A}"
            alt=""
          />`}Vt.innerHTML=Rt();const Gt=document.querySelector(".catalogGarminMarq");function Ht(){return`<source
            srcset="
              ${pt} 1x,
              ${xt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${dt} 1x,
              ${bt} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${u} 1x,
              ${Mt} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${u}"
            alt=""
          />`}Gt.innerHTML=Ht();const Zt=document.querySelector(".catalogGarminDescent");function Qt(){return`<source
            srcset="
              ${wt} 1x,
              ${v} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${kt} 1x,
              ${Wt} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${g} 1x,
              ${ft} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${g}"
            alt=""
          />`}Zt.innerHTML=Qt();const zt=document.querySelector(".catalogGarminD2");function Pt(){return`<source
            srcset="
              ${Ct} 1x,
              ${v} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${vt} 1x,
              ${yt} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${p} 1x,
              ${Tt} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${p}"
            alt=""
          />`}zt.innerHTML=Pt();const qt=document.querySelector(".catalogGarminFenix6");function Kt(){return`<source
            srcset="
              ${Dt} 1x,
              ${It} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${x} 1x,
              ${Bt} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${x}"
            alt=""
          />`}qt.innerHTML=Kt();const Ut=document.querySelector(".catalogTubeWatchS42");function Jt(){return`<source
            srcset="
              ${St} 1x,
              ${Et} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${d}  1x,
              ${Lt} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${d}"
            alt=""
          />`}Ut.innerHTML=Jt();const Yt="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",jt="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",Xt="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",Ft="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",b="data:image/png;base64,",Nt="/WatchCharm/assets/photo-watch01-mobile@2x-CMkT6Cb-.png",Ot="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",$t="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",_t="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",te="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",M="data:image/png;base64,",ee="/WatchCharm/assets/photo-watch02-mobile@2x-BS2zyQn7.png",ae="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",se="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",ce="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",ne="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",w="data:image/png;base64,",oe="/WatchCharm/assets/photo-watch03-mobile@2x-DGmTBPWB.png",re="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",ie="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",he="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",me="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",k="data:image/png;base64,",le="/WatchCharm/assets/photo-watch04-mobile@2x-hm4ppRAL.png",Ae="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",ue="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",ge="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",pe="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",W="/WatchCharm/assets/photo-big-watch-mobile-CHvM7Ojo.png",xe="/WatchCharm/assets/photo-big-watch-mobile.png@2x-Ckojk9-h.png",de=document.querySelector(".saleWatch01");function be(){return`<source
                srcset="
                  ${Yt} 1x,
                  ${jt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Xt} 1x,
                  ${Ft} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${b} 1x,
                  ${Nt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${b}" alt="" />`}de.innerHTML=be();const Me=document.querySelector(".saleWatch02");function we(){return`<source
                srcset="
                  ${Ot} 1x,
                  ${$t} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${_t} 1x,
                  ${te} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${M} 1x,
                  ${ee} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${M}" alt="" />`}Me.innerHTML=we();const ke=document.querySelector(".saleWatch03");function We(){return`<source
                srcset="
                  ${ae} 1x,
                  ${se} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${ce} 1x,
                  ${ne} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${w} 1x,
                  ${oe} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${w}" alt="" />`}ke.innerHTML=We();const fe=document.querySelector(".saleWatch04");function Ce(){return` <source
                srcset="
                  ${re} 1x,
                  ${ie} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${he} 1x,
                  ${me} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${k} 1x,
                  ${le} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${k}" alt="" />`}fe.innerHTML=Ce();const ve=document.querySelector(".saleBigWatch");function ye(){return`<source
              srcset="
                ${Ae} 1x,
                ${ue} 2x
              "
              media="(min-width: 1280px)"
            />
            <source
              srcset="
                ${ge} 1x,
                ${pe} 2x
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${W} 1x,
                ${xe} 2x
              "
              media="(min-width: 375px)"
            />
            <img src="${W}" alt="" />`}ve.innerHTML=ye();const f="data:image/png;base64,",Te="/WatchCharm/assets/john-wax@2x-SBpjfyTe.png",De="data:image/png;base64,",Ie="/WatchCharm/assets/victoria-romashenko@2x-C2CY2F_l.png",Be="data:image/png;base64,",Se="/WatchCharm/assets/anastasia-trachuk@2x-D7g_BFBX.png",Ee=document.querySelector(".reviewsJohnWax");function Le(){return`<img
          class="reviews-avatar"
          srcset="
            ${f} 1x,
            ${Te} 2x
          "
          src="${f}"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">John Wax</h3>
        <p class="reviews-text">
          Buying a watch from was the best choice for me. The design is
          perfectly combined with high quality materials. The strap fits nicely
          on the hand, and the mechanism works flawlessly.
        </p>`}Ee.innerHTML=Le();const Ve=document.querySelector(".reviewsVictoriaRomashenko");function Re(){return`<img
          class="reviews-avatar"
          srcset="
            ${De} 1x,
            ${Ie} 2x
          "
          src="../img/reviews/victoria-romashenko.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Victoria Romashenko</h3>
        <p class="reviews-text">
          Can't take my eyes off my watch from. Every time I put it on, it is
          admired by those around me. The build quality and attention to detail
          make it a real gem among my accessories.
        </p>`}Ve.innerHTML=Re();const Ge=document.querySelector(".reviewsAnastasiaTrachuk");function He(){return`<img
          class="reviews-avatar"
          srcset="
            ${Be} 1x,
            ${Se} 2x
          "
          src="../img/reviews/anastasia-trachuk.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Anastasia Trachuk</h3>
        <p class="reviews-text">
          My watch from is a real masterpiece. Every detail impresses with the
          high quality of execution. Special features like [specify features]
          make it an indispensable companion in my daily life.
        </p>`}Ge.innerHTML=He();const Ze="/WatchCharm/assets/photo-watch-desktop-DDmQbKlm.jpg",Qe="/WatchCharm/assets/photo-watch-desktop@2x-D32LJr8P.jpg",ze="/WatchCharm/assets/photo-watch-tablet-CuaPopDZ.jpg",Pe="/WatchCharm/assets/photo-watch-tablet@2x-CCm7oC6U.jpg",C="/WatchCharm/assets/photo-watch-mobile-DqB8AVQE.jpg",qe="/WatchCharm/assets/photo-watch-mobile@2x-C4EWnBiJ.jpg",Ke=document.querySelector(".contactPhotoWatch");function Ue(){return`<source
          srcset="
            ${Ze} 1x,
            ${Qe} 2x
          "
          media="(min-width: 1280px)"
        />
        <source
          srcset="
            ${ze} 1x,
            ${Pe} 2x
          "
          media="(min-width: 768px)"
        />
        <source
          srcset="
            ${C} 1x,
            ${qe} 2x
          "
          media="(min-width: 375px)"
        />
        <img
          class="contact-us-picture"
          src="${C}"
          alt=""
        />`}Ke.innerHTML=Ue();(()=>{const c=document.querySelector(".js-menu-container"),a=document.querySelector(".js-open-menu"),n=document.querySelector(".js-close-menu"),s=()=>{const t=a.getAttribute("aria-expanded")==="true"||!1;a.setAttribute("aria-expanded",!t),c.classList.toggle("is-open");const e=t?"enableBodyScroll":"disableBodyScroll";bodyScrollLock[e](document.body)};a.addEventListener("click",s),n.addEventListener("click",s),window.matchMedia("(min-width: 768px)").addEventListener("change",t=>{t.matches&&(c.classList.remove("is-open"),a.setAttribute("aria-expanded",!1),bodyScrollLock.enableBodyScroll(document.body))})})();
//# sourceMappingURL=index.js.map
