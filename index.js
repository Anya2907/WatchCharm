(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const o of e.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&s(o)}).observe(document,{childList:!0,subtree:!0});function n(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function s(t){if(t.ep)return;t.ep=!0;const e=n(t);fetch(t.href,e)}})();const r="/WatchCharm/assets/icons-zpEVHya5.svg#icon-facebook",i="/WatchCharm/assets/icons-zpEVHya5.svg#icon-instagram",h="/WatchCharm/assets/icons-zpEVHya5.svg#icon-youtube",y="/WatchCharm/assets/icons-zpEVHya5.svg#icon-menu-burger",T=document.querySelector(".headerIconFacebook");function D(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${r}"></use>
            </svg>
          </a>`}T.innerHTML=D();const I=document.querySelector(".headerIconInstagram");function B(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${i}"></use>
            </svg>
          </a>`}I.innerHTML=B();const S=document.querySelector(".headerIconYoutube");function E(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${h}"></use>
            </svg>
          </a>`}S.innerHTML=E();const L=document.querySelector(".headerBurger");function V(){return`<svg class="menu-burger-icon" width="34" height="34">
          <use href="${y}"></use>
        </svg>`}L.innerHTML=V();const R="/WatchCharm/assets/photo-desktop-DZgTKgeg.png",G="/WatchCharm/assets/photo-desktop@2x-DFzG8mW0.png",H="/WatchCharm/assets/photo-tablet-BmO2Vl-6.png",Z="/WatchCharm/assets/photo-tablet@2x-EcloSsxX.png",m="data:image/png;base64,",Q="/WatchCharm/assets/photo-mobile@2x-CYpZwyro.png",z=document.querySelector(".hero-photo");function P(){return`<source
            srcset="
              ${R} 1x,
              ${G} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${H} 1x,
              ${Z} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${m} 1x,
              ${Q} 2x
            "
            media="(min-width: 375px)"
          />
          <img src="${m}" alt="" />`}z.innerHTML=P();const q="/WatchCharm/assets/watch-desktop-BoveKM56.jpg",K="/WatchCharm/assets/watch-desktop@2x-CN_DlGJv.jpg",U="/WatchCharm/assets/watch-tablet-DzWGp1S_.jpg",J="/WatchCharm/assets/watch-tablet@2x-h8tUE2WX.jpg",l="/WatchCharm/assets/watch-mobile-gUDyuoK7.jpg",Y="/WatchCharm/assets/watch-mobile@2x-DC3-hsv0.jpg",j=document.querySelector(".about-us-watch");function X(){return` <source
        srcset="
          ${q} 1x,
          ${K} 2x
        "
        media="(min-width: 1280px)"
      />
      <source
        srcset="
          ${U} 1x,
          ${J} 2x
        "
        media="(min-width: 768px)"
      />
      <source
        srcset="
        ${l} 1x,
        ${Y} 2x"
        media="(min-width: 375px)"
      />
      <img src="${l}" alt="" />`}j.innerHTML=X();const F="/WatchCharm/assets/garmin-fenix-7X-BGyNroPE.png",N="/WatchCharm/assets/garmin-fenix-7X@2x-CLNKzvYI.png",O="/WatchCharm/assets/garmin-fenix-7X-BYzg8P9N.png",$="/WatchCharm/assets/garmin-fenix-7X@2x-DgvqIYlC.png",A="/WatchCharm/assets/garmin-fenix-BtUZFNqa.png",_="/WatchCharm/assets/garmin-fenix@2x-DgAmOKLa.png",tt="/WatchCharm/assets/garmin-marq-D7Zoid31.png",et="/WatchCharm/assets/garmin-marq@2x-D2WqiF2K.png",at="/WatchCharm/assets/garmin-marq-_WDLzsmf.png",st="/WatchCharm/assets/garmin-marq@2x-BazMqD_o.png",u="/WatchCharm/assets/garmin-marq-S2plHlxp.png",ct="/WatchCharm/assets/garmin-marq@2x-CgjFqqcK.png",nt="/WatchCharm/assets/garmin-descent-CatdJNn4.png",v="/WatchCharm/assets/garmin-D2@2x-CTMGRPFG.png",ot="/WatchCharm/assets/garmin-descent-RgCLEepd.png",rt="/WatchCharm/assets/garmin-descent@2x-DJlmk73G.png",g="/WatchCharm/assets/garmin-descent-D058-41F.png",it="/WatchCharm/assets/garmin-descent@2x-DSrl5rur.png",ht="/WatchCharm/assets/garmin-D2-BPYVYLKf.png",mt="/WatchCharm/assets/garmin-D2-CdkTsl9X.png",lt="/WatchCharm/assets/garmin-D2@2x-uwK-wnBs.png",p="/WatchCharm/assets/garmin-D2-B_lN0l0e.png",At="/WatchCharm/assets/garmin-D2@2x-D4h7HLtv.png",ut="/WatchCharm/assets/garmin-fenix-6-_fQUo3lG.png",gt="/WatchCharm/assets/garmin-fenix-6@2x-Du5HDKBW.png",x="/WatchCharm/assets/garmin-fenix-6-D5xVD_H4.png",pt="/WatchCharm/assets/garmin-fenix-6@2x-DmzbNT7X.png",xt="/WatchCharm/assets/tube-watch-S42-ba4c6ajU.png",dt="/WatchCharm/assets/tube-watch-S42@2x-CaBP822J.png",d="/WatchCharm/assets/tube-watch-S42-CK21dafh.png",bt="/WatchCharm/assets/tube-watch-S42@2x-DFR-76cY.png",Mt=document.querySelector(".catalogGarminFenix7X");function wt(){return`<source
            srcset="
              ${F} 1x,
              ${N} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${O} 1x,
              ${$} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${A} 1x,
              ${_} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${A}"
            alt=""
          />`}Mt.innerHTML=wt();const kt=document.querySelector(".catalogGarminMarq");function Wt(){return`<source
            srcset="
              ${tt} 1x,
              ${et} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${at} 1x,
              ${st} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${u} 1x,
              ${ct} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${u}"
            alt=""
          />`}kt.innerHTML=Wt();const ft=document.querySelector(".catalogGarminDescent");function Ct(){return`<source
            srcset="
              ${nt} 1x,
              ${v} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${ot} 1x,
              ${rt} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${g} 1x,
              ${it} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${g}"
            alt=""
          />`}ft.innerHTML=Ct();const vt=document.querySelector(".catalogGarminD2");function yt(){return`<source
            srcset="
              ${ht} 1x,
              ${v} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${mt} 1x,
              ${lt} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${p} 1x,
              ${At} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${p}"
            alt=""
          />`}vt.innerHTML=yt();const Tt=document.querySelector(".catalogGarminFenix6");function Dt(){return`<source
            srcset="
              ${ut} 1x,
              ${gt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${x} 1x,
              ${pt} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${x}"
            alt=""
          />`}Tt.innerHTML=Dt();const It=document.querySelector(".catalogTubeWatchS42");function Bt(){return`<source
            srcset="
              ${xt} 1x,
              ${dt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${d}  1x,
              ${bt} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${d}"
            alt=""
          />`}It.innerHTML=Bt();const St="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",Et="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",Lt="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",Vt="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",b="data:image/png;base64,",Rt="/WatchCharm/assets/photo-watch01-mobile@2x-CMkT6Cb-.png",Gt="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",Ht="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",Zt="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",Qt="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",M="data:image/png;base64,",zt="/WatchCharm/assets/photo-watch02-mobile@2x-BS2zyQn7.png",Pt="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",qt="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",Kt="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",Ut="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",w="data:image/png;base64,",Jt="/WatchCharm/assets/photo-watch03-mobile@2x-DGmTBPWB.png",Yt="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",jt="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",Xt="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",Ft="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",k="data:image/png;base64,",Nt="/WatchCharm/assets/photo-watch04-mobile@2x-hm4ppRAL.png",Ot="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",$t="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",_t="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",te="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",W="/WatchCharm/assets/photo-big-watch-mobile-CHvM7Ojo.png",ee="/WatchCharm/assets/photo-big-watch-mobile.png@2x-Ckojk9-h.png",ae=document.querySelector(".saleWatch01");function se(){return`<source
                srcset="
                  ${St} 1x,
                  ${Et} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Lt} 1x,
                  ${Vt} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${b} 1x,
                  ${Rt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${b}" alt="" />`}ae.innerHTML=se();const ce=document.querySelector(".saleWatch02");function ne(){return`<source
                srcset="
                  ${Gt} 1x,
                  ${Ht} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Zt} 1x,
                  ${Qt} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${M} 1x,
                  ${zt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${M}" alt="" />`}ce.innerHTML=ne();const oe=document.querySelector(".saleWatch03");function re(){return`<source
                srcset="
                  ${Pt} 1x,
                  ${qt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Kt} 1x,
                  ${Ut} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${w} 1x,
                  ${Jt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${w}" alt="" />`}oe.innerHTML=re();const ie=document.querySelector(".saleWatch04");function he(){return` <source
                srcset="
                  ${Yt} 1x,
                  ${jt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Xt} 1x,
                  ${Ft} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${k} 1x,
                  ${Nt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${k}" alt="" />`}ie.innerHTML=he();const me=document.querySelector(".saleBigWatch");function le(){return`<source
              srcset="
                ${Ot} 1x,
                ${$t} 2x
              "
              media="(min-width: 1280px)"
            />
            <source
              srcset="
                ${_t} 1x,
                ${te} 2x
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${W} 1x,
                ${ee} 2x
              "
              media="(min-width: 375px)"
            />
            <img src="${W}" alt="" />`}me.innerHTML=le();const f="data:image/png;base64,",Ae="/WatchCharm/assets/john-wax@2x-SBpjfyTe.png",ue="data:image/png;base64,",ge="/WatchCharm/assets/victoria-romashenko@2x-C2CY2F_l.png",pe="data:image/png;base64,",xe="/WatchCharm/assets/anastasia-trachuk@2x-D7g_BFBX.png",de=document.querySelector(".reviewsJohnWax");function be(){return`<img
          class="reviews-avatar"
          srcset="
            ${f} 1x,
            ${Ae} 2x
          "
          src="${f}"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">John Wax</h3>
        <p class="reviews-text">
          Buying a watch from was the best choice for me. The design is
          perfectly combined with high quality materials. The strap fits nicely
          on the hand, and the mechanism works flawlessly.
        </p>`}de.innerHTML=be();const Me=document.querySelector(".reviewsVictoriaRomashenko");function we(){return`<img
          class="reviews-avatar"
          srcset="
            ${ue} 1x,
            ${ge} 2x
          "
          src="../img/reviews/victoria-romashenko.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Victoria Romashenko</h3>
        <p class="reviews-text">
          Can't take my eyes off my watch from. Every time I put it on, it is
          admired by those around me. The build quality and attention to detail
          make it a real gem among my accessories.
        </p>`}Me.innerHTML=we();const ke=document.querySelector(".reviewsAnastasiaTrachuk");function We(){return`<img
          class="reviews-avatar"
          srcset="
            ${pe} 1x,
            ${xe} 2x
          "
          src="../img/reviews/anastasia-trachuk.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Anastasia Trachuk</h3>
        <p class="reviews-text">
          My watch from is a real masterpiece. Every detail impresses with the
          high quality of execution. Special features like [specify features]
          make it an indispensable companion in my daily life.
        </p>`}ke.innerHTML=We();const fe="/WatchCharm/assets/photo-watch-desktop-DDmQbKlm.jpg",Ce="/WatchCharm/assets/photo-watch-desktop@2x-D32LJr8P.jpg",ve="/WatchCharm/assets/photo-watch-tablet-CuaPopDZ.jpg",ye="/WatchCharm/assets/photo-watch-tablet@2x-CCm7oC6U.jpg",C="/WatchCharm/assets/photo-watch-mobile-DqB8AVQE.jpg",Te="/WatchCharm/assets/photo-watch-mobile@2x-C4EWnBiJ.jpg",De=document.querySelector(".contactPhotoWatch");function Ie(){return`<source
          srcset="
            ${fe} 1x,
            ${Ce} 2x
          "
          media="(min-width: 1280px)"
        />
        <source
          srcset="
            ${ve} 1x,
            ${ye} 2x
          "
          media="(min-width: 768px)"
        />
        <source
          srcset="
            ${C} 1x,
            ${Te} 2x
          "
          media="(min-width: 375px)"
        />
        <img
          class="contact-us-picture"
          src="${C}"
          alt=""
        />`}De.innerHTML=Ie();const Be=document.querySelector(".footerIconFacebook");function Se(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${r}"
                    ></use>
                  </svg>
                </a>`}Be.innerHTML=Se();const Ee=document.querySelector(".footerIconInstagram");function Le(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${i}"
                    ></use>
                  </svg>
                </a>`}Ee.innerHTML=Le();const Ve=document.querySelector(".footerIconYoutube");function Re(){return`<a class="list-link-icons footer-list-link-icons" href="">
                  <svg class="list-item-icon" width="18" height="18">
                    <use
                      href="${h}"
                    ></use>
                  </svg>
                </a>`}Ve.innerHTML=Re();const Ge="/WatchCharm/assets/icons-zpEVHya5.svg#icon-close",He=document.querySelector(".mobileMenuCloseIcon");function Ze(){return`<svg class="mobile-menu-close-icon" width="34" height="34">
        <use href="${Ge}"></use>
      </svg>`}He.innerHTML=Ze();const Qe=document.querySelector(".mobileMenuIconFacebook");function ze(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${r}"></use>
          </svg>
        </a>`}Qe.innerHTML=ze();const Pe=document.querySelector(".mobileMenuIconInstagram");function qe(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${i}"></use>
          </svg>
        </a>`}Pe.innerHTML=qe();const Ke=document.querySelector(".mobileMenuIconYoutube");function Ue(){return`<a class="mobile-menu-list-link-icons" href="">
          <svg class="mobile-list-item-icon" width="18" height="18">
            <use href="${h}"></use>
          </svg>
        </a>`}Ke.innerHTML=Ue();(()=>{const c=document.querySelector(".js-menu-container"),a=document.querySelector(".js-open-menu"),n=document.querySelector(".js-close-menu"),s=()=>{const t=a.getAttribute("aria-expanded")==="true"||!1;a.setAttribute("aria-expanded",!t),c.classList.toggle("is-open");const e=t?"enableBodyScroll":"disableBodyScroll";bodyScrollLock[e](document.body)};a.addEventListener("click",s),n.addEventListener("click",s),window.matchMedia("(min-width: 768px)").addEventListener("change",t=>{t.matches&&(c.classList.remove("is-open"),a.setAttribute("aria-expanded",!1),bodyScrollLock.enableBodyScroll(document.body))})})();
//# sourceMappingURL=index.js.map
