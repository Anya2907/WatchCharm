(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function n(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(t){if(t.ep)return;t.ep=!0;const a=n(t);fetch(t.href,a)}})();const f="/WatchCharm/assets/icons-zpEVHya5.svg#icon-facebook",v="/WatchCharm/assets/icons-zpEVHya5.svg#icon-youtube",D=document.querySelector(".headerIconFacebook");function T(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${f}"></use>
            </svg>
          </a>`}D.innerHTML=T();const M=document.querySelector(".headerIconInstagram");function y(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${M}"></use>
            </svg>
          </a>`}M.innerHTML=y();const B=document.querySelector(".headerIconYoutube");function I(){return`<a class="list-link-icons menu-list-link-icons" href="">
            <svg class="list-item-icon" width="20" height="20">
              <use href="${v}"></use>
            </svg>
          </a>`}B.innerHTML=I();const S="/WatchCharm/assets/photo-desktop-DZgTKgeg.png",E="/WatchCharm/assets/photo-desktop@2x-DFzG8mW0.png",R="/WatchCharm/assets/photo-tablet-BmO2Vl-6.png",L="/WatchCharm/assets/photo-tablet@2x-EcloSsxX.png",o="data:image/png;base64,",V="/WatchCharm/assets/photo-mobile@2x-CYpZwyro.png",G=document.querySelector(".hero-photo");function Z(){return`<source
            srcset="
              ${S} 1x,
              ${E} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${R} 1x,
              ${L} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${o} 1x,
              ${V} 2x
            "
            media="(min-width: 375px)"
          />
          <img src="${o}" alt="" />`}G.innerHTML=Z();const Q="/WatchCharm/assets/watch-desktop-BoveKM56.jpg",P="/WatchCharm/assets/watch-desktop@2x-CN_DlGJv.jpg",H="/WatchCharm/assets/watch-tablet-DzWGp1S_.jpg",z="/WatchCharm/assets/watch-tablet@2x-h8tUE2WX.jpg",i="/WatchCharm/assets/watch-mobile-gUDyuoK7.jpg",K="/WatchCharm/assets/watch-mobile@2x-DC3-hsv0.jpg",U=document.querySelector(".about-us-watch");function q(){return` <source
        srcset="
          ${Q} 1x,
          ${P} 2x
        "
        media="(min-width: 1280px)"
      />
      <source
        srcset="
          ${H} 1x,
          ${z} 2x
        "
        media="(min-width: 768px)"
      />
      <source
        srcset="
        ${i} 1x,
        ${K} 2x"
        media="(min-width: 375px)"
      />
      <img src="${i}" alt="" />`}U.innerHTML=q();const J="/WatchCharm/assets/garmin-fenix-7X-BGyNroPE.png",j="/WatchCharm/assets/garmin-fenix-7X@2x-CLNKzvYI.png",Y="/WatchCharm/assets/garmin-fenix-7X-BYzg8P9N.png",X="/WatchCharm/assets/garmin-fenix-7X@2x-DgvqIYlC.png",h="/WatchCharm/assets/garmin-fenix-BtUZFNqa.png",N="/WatchCharm/assets/garmin-fenix@2x-DgAmOKLa.png",O="/WatchCharm/assets/garmin-marq-D7Zoid31.png",F="/WatchCharm/assets/garmin-marq@2x-D2WqiF2K.png",$="/WatchCharm/assets/garmin-marq-_WDLzsmf.png",_="/WatchCharm/assets/garmin-marq@2x-BazMqD_o.png",m="/WatchCharm/assets/garmin-marq-S2plHlxp.png",tt="/WatchCharm/assets/garmin-marq@2x-CgjFqqcK.png",at="/WatchCharm/assets/garmin-descent-CatdJNn4.png",C="/WatchCharm/assets/garmin-D2@2x-CTMGRPFG.png",et="/WatchCharm/assets/garmin-descent-RgCLEepd.png",st="/WatchCharm/assets/garmin-descent@2x-DJlmk73G.png",A="/WatchCharm/assets/garmin-descent-D058-41F.png",ct="/WatchCharm/assets/garmin-descent@2x-DSrl5rur.png",nt="/WatchCharm/assets/garmin-D2-BPYVYLKf.png",rt="/WatchCharm/assets/garmin-D2-CdkTsl9X.png",ot="/WatchCharm/assets/garmin-D2@2x-uwK-wnBs.png",l="/WatchCharm/assets/garmin-D2-B_lN0l0e.png",it="/WatchCharm/assets/garmin-D2@2x-D4h7HLtv.png",ht="/WatchCharm/assets/garmin-fenix-6-_fQUo3lG.png",mt="/WatchCharm/assets/garmin-fenix-6@2x-Du5HDKBW.png",g="/WatchCharm/assets/garmin-fenix-6-D5xVD_H4.png",At="/WatchCharm/assets/garmin-fenix-6@2x-DmzbNT7X.png",lt="/WatchCharm/assets/tube-watch-S42-ba4c6ajU.png",gt="/WatchCharm/assets/tube-watch-S42@2x-CaBP822J.png",p="/WatchCharm/assets/tube-watch-S42-CK21dafh.png",pt="/WatchCharm/assets/tube-watch-S42@2x-DFR-76cY.png",ut=document.querySelector(".catalogGarminFenix7X");function xt(){return`<source
            srcset="
              ${J} 1x,
              ${j} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${Y} 1x,
              ${X} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${h} 1x,
              ${N} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${h}"
            alt=""
          />`}ut.innerHTML=xt();const dt=document.querySelector(".catalogGarminMarq");function bt(){return`<source
            srcset="
              ${O} 1x,
              ${F} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${$} 1x,
              ${_} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${m} 1x,
              ${tt} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${m}"
            alt=""
          />`}dt.innerHTML=bt();const wt=document.querySelector(".catalogGarminDescent");function Wt(){return`<source
            srcset="
              ${at} 1x,
              ${C} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${et} 1x,
              ${st} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${A} 1x,
              ${ct} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${A}"
            alt=""
          />`}wt.innerHTML=Wt();const kt=document.querySelector(".catalogGarminD2");function Mt(){return`<source
            srcset="
              ${nt} 1x,
              ${C} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${rt} 1x,
              ${ot} 2x
            "
            media="(min-width: 768px)"
          />
          <source
            srcset="
              ${l} 1x,
              ${it} 2x
            "
            media="(min-width: 375px)"
          />
          <img
            class="catalog-picture"
            src="${l}"
            alt=""
          />`}kt.innerHTML=Mt();const Ct=document.querySelector(".catalogGarminFenix6");function ft(){return`<source
            srcset="
              ${ht} 1x,
              ${mt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${g} 1x,
              ${At} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${g}"
            alt=""
          />`}Ct.innerHTML=ft();const vt=document.querySelector(".catalogTubeWatchS42");function Dt(){return`<source
            srcset="
              ${lt} 1x,
              ${gt} 2x
            "
            media="(min-width: 1280px)"
          />
          <source
            srcset="
              ${p}  1x,
              ${pt} 2x
            "
            media="(min-width: 768px)"
          />
          <img
            class="catalog-picture"
            src="${p}"
            alt=""
          />`}vt.innerHTML=Dt();const Tt="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",yt="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",Bt="/WatchCharm/assets/photo-watch01-tablet-BHd6lBFh.png",It="/WatchCharm/assets/photo-watch01-tablet@2x-Cnigrlj3.png",u="data:image/png;base64,",St="/WatchCharm/assets/photo-watch01-mobile@2x-CMkT6Cb-.png",Et="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",Rt="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",Lt="/WatchCharm/assets/photo-watch02-tablet-BAzifCN5.png",Vt="/WatchCharm/assets/photo-watch02-tablet@2x-BZL2kT1j.png",x="data:image/png;base64,",Gt="/WatchCharm/assets/photo-watch02-mobile@2x-BS2zyQn7.png",Zt="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",Qt="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",Pt="/WatchCharm/assets/photo-watch03-tablet-CDjQ6Uub.png",Ht="/WatchCharm/assets/photo-watch03-tablet@2x-0kyItJ9t.png",d="data:image/png;base64,",zt="/WatchCharm/assets/photo-watch03-mobile@2x-DGmTBPWB.png",Kt="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",Ut="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",qt="/WatchCharm/assets/photo-watch04-tablet-C8VRCFn8.png",Jt="/WatchCharm/assets/photo-watch04-tablet@2x-r5rlsjyl.png",b="data:image/png;base64,",jt="/WatchCharm/assets/photo-watch04-mobile@2x-hm4ppRAL.png",Yt="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",Xt="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",Nt="/WatchCharm/assets/photo-big-watch-tablet-BDDM0Dcc.png",Ot="/WatchCharm/assets/photo-big-watch-tablet@2x-Cmt4Ihsp.png",w="/WatchCharm/assets/photo-big-watch-mobile-CHvM7Ojo.png",Ft="/WatchCharm/assets/photo-big-watch-mobile.png@2x-Ckojk9-h.png",$t=document.querySelector(".saleWatch01");function _t(){return`<source
                srcset="
                  ${Tt} 1x,
                  ${yt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Bt} 1x,
                  ${It} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${u} 1x,
                  ${St} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${u}" alt="" />`}$t.innerHTML=_t();const ta=document.querySelector(".saleWatch02");function aa(){return`<source
                srcset="
                  ${Et} 1x,
                  ${Rt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Lt} 1x,
                  ${Vt} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${x} 1x,
                  ${Gt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${x}" alt="" />`}ta.innerHTML=aa();const ea=document.querySelector(".saleWatch03");function sa(){return`<source
                srcset="
                  ${Zt} 1x,
                  ${Qt} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${Pt} 1x,
                  ${Ht} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${d} 1x,
                  ${zt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${d}" alt="" />`}ea.innerHTML=sa();const ca=document.querySelector(".saleWatch04");function na(){return` <source
                srcset="
                  ${Kt} 1x,
                  ${Ut} 2x
                "
                media="(min-width: 1280px)"
              />
              <source
                srcset="
                  ${qt} 1x,
                  ${Jt} 2x
                "
                media="(min-width: 768px)"
              />
              <source
                srcset="
                  ${b} 1x,
                  ${jt} 2x
                "
                media="(min-width: 375px)"
              />
              <img src="${b}" alt="" />`}ca.innerHTML=na();const ra=document.querySelector(".saleBigWatch");function oa(){return`<source
              srcset="
                ${Yt} 1x,
                ${Xt} 2x
              "
              media="(min-width: 1280px)"
            />
            <source
              srcset="
                ${Nt} 1x,
                ${Ot} 2x
              "
              media="(min-width: 768px)"
            />
            <source
              srcset="
                ${w} 1x,
                ${Ft} 2x
              "
              media="(min-width: 375px)"
            />
            <img src="${w}" alt="" />`}ra.innerHTML=oa();const W="data:image/png;base64,",ia="/WatchCharm/assets/john-wax@2x-SBpjfyTe.png",ha="data:image/png;base64,",ma="/WatchCharm/assets/victoria-romashenko@2x-C2CY2F_l.png",Aa="data:image/png;base64,",la="/WatchCharm/assets/anastasia-trachuk@2x-D7g_BFBX.png",ga=document.querySelector(".reviewsJohnWax");function pa(){return`<img
          class="reviews-avatar"
          srcset="
            ${W} 1x,
            ${ia} 2x
          "
          src="${W}"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">John Wax</h3>
        <p class="reviews-text">
          Buying a watch from was the best choice for me. The design is
          perfectly combined with high quality materials. The strap fits nicely
          on the hand, and the mechanism works flawlessly.
        </p>`}ga.innerHTML=pa();const ua=document.querySelector(".reviewsVictoriaRomashenko");function xa(){return`<img
          class="reviews-avatar"
          srcset="
            ${ha} 1x,
            ${ma} 2x
          "
          src="../img/reviews/victoria-romashenko.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Victoria Romashenko</h3>
        <p class="reviews-text">
          Can't take my eyes off my watch from. Every time I put it on, it is
          admired by those around me. The build quality and attention to detail
          make it a real gem among my accessories.
        </p>`}ua.innerHTML=xa();const da=document.querySelector(".reviewsAnastasiaTrachuk");function ba(){return`<img
          class="reviews-avatar"
          srcset="
            ${Aa} 1x,
            ${la} 2x
          "
          src="../img/reviews/anastasia-trachuk.png"
          alt=""
        />
        <h3 class="reviews-title-name-avatar">Anastasia Trachuk</h3>
        <p class="reviews-text">
          My watch from is a real masterpiece. Every detail impresses with the
          high quality of execution. Special features like [specify features]
          make it an indispensable companion in my daily life.
        </p>`}da.innerHTML=ba();const wa="/WatchCharm/assets/photo-watch-desktop-DDmQbKlm.jpg",Wa="/WatchCharm/assets/photo-watch-desktop@2x-D32LJr8P.jpg",ka="/WatchCharm/assets/photo-watch-tablet-CuaPopDZ.jpg",Ma="/WatchCharm/assets/photo-watch-tablet@2x-CCm7oC6U.jpg",k="/WatchCharm/assets/photo-watch-mobile-DqB8AVQE.jpg",Ca="/WatchCharm/assets/photo-watch-mobile@2x-C4EWnBiJ.jpg",fa=document.querySelector(".contactPhotoWatch");function va(){return`<source
          srcset="
            ${wa} 1x,
            ${Wa} 2x
          "
          media="(min-width: 1280px)"
        />
        <source
          srcset="
            ${ka} 1x,
            ${Ma} 2x
          "
          media="(min-width: 768px)"
        />
        <source
          srcset="
            ${k} 1x,
            ${Ca} 2x
          "
          media="(min-width: 375px)"
        />
        <img
          class="contact-us-picture"
          src="${k}"
          alt=""
        />`}fa.innerHTML=va();(()=>{const c=document.querySelector(".js-menu-container"),e=document.querySelector(".js-open-menu"),n=document.querySelector(".js-close-menu"),s=()=>{const t=e.getAttribute("aria-expanded")==="true"||!1;e.setAttribute("aria-expanded",!t),c.classList.toggle("is-open");const a=t?"enableBodyScroll":"disableBodyScroll";bodyScrollLock[a](document.body)};e.addEventListener("click",s),n.addEventListener("click",s),window.matchMedia("(min-width: 768px)").addEventListener("change",t=>{t.matches&&(c.classList.remove("is-open"),e.setAttribute("aria-expanded",!1),bodyScrollLock.enableBodyScroll(document.body))})})();
//# sourceMappingURL=index.js.map
